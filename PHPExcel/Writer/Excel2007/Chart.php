...
class PHPExcel_Writer_Excel2007_Chart extends PHPExcel_Writer_Excel2007_WriterPart
{
    ...
    private function writePlotArea(PHPExcel_Chart_PlotArea $plotArea, PHPExcel_Chart_Title $xAxisLabel = null, PHPExcel_Chart_Title $yAxisLabel = null, $objWriter, PHPExcel_Worksheet $pSheet, PHPExcel_Chart_Axis $xAxis, PHPExcel_Chart_Axis $yAxis, PHPExcel_Chart_GridLines $majorGridlines, PHPExcel_Chart_GridLines $minorGridlines,PHPExcel_Chart_Title $secondaryYAxisLabel = NULL)
    {
        ...
        foreach ($chartTypes as $chartType) {
            ...
            if ($chartType === PHPExcel_Chart_DataSeries::TYPE_LINECHART) {
                ...
                $plotGroup = $plotArea->getPlotGroupByIndex(0);
                if (!empty($plotGroup->customStyle['barGapWidth'])) {
                    $objWriter->writeAttribute('val', $plotGroup->customStyle['barGapWidth']);
                } else {
    ...
    private function writeCategoryAxis($objWriter, PHPExcel_Chart_PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries, $xAxis, $yAxis, $secondary = false)
    {
        ...
        // x轴标签样式
        $plotGroup = $plotArea->getPlotGroupByIndex(0);
        if (!empty($plotGroup->customStyle['fontSize'])) {
            $objWriter->startElement('c:txPr');
            $objWriter->startElement('a:bodyPr');
            $objWriter->endElement();
            ...
            $objWriter->startElement('a:defRPr');
            $objWriter->writeAttribute('sz', $plotGroup->customStyle['fontSize']);
            $objWriter->endElement();
            $objWriter->endElement();
    ...
    private function writeValueAxis($objWriter, PHPExcel_Chart_PlotArea $plotArea, $yAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries, $xAxis, $yAxis, $majorGridlines, $minorGridlines, $secondary = false)
    {
        ...
        if ($secondary && !empty($xAxis->secondaryStyle['maximum'])) {
            $objWriter->startElement('c:max');
            $objWriter->writeAttribute('val', $xAxis->secondaryStyle['maximum']);
            $objWriter->endElement();
        } else {
        ...
        if ($secondary && !empty($xAxis->secondaryStyle['minimum'])) {
            $objWriter->startElement('c:min');
            $objWriter->writeAttribute('val', $xAxis->secondaryStyle['minimum']);
            $objWriter->endElement();
        } else {
        ...
        $objWriter->startElement('c:tickLblPos');
        if ($secondary && !empty($xAxis->secondaryStyle['axis_labels'])) {
            $objWriter->writeAttribute('val', $xAxis->secondaryStyle['axis_labels']);
        } else {
        ...
        $plotGroup = $plotArea->getPlotSecondaryGroupByIndex(0);
        if (!empty($plotGroup->customStyle['hide_label_border'])) {
        ...
        if ($id1 > 0) {
            ...
            {
                $plotGroup = $plotArea->getPlotGroupByIndex(0);
                if ($plotGroup->customStyle['multi_series_with_secondary']) {
                    $objWriter->writeAttribute('val', $id1);
                } else {
                    $objWriter->writeAttribute('val', $id2);
                }
    ...
    private function writePlotGroup($plotGroup, $groupType, $objWriter, &$catIsMultiLevelSeries, &$valIsMultiLevelSeries, &$plotGroupingType, PHPExcel_Worksheet $pSheet)
    {
        ...
        foreach ($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
            ...
            if (($groupType == PHPExcel_Chart_DataSeries::TYPE_LINECHART) || ($groupType == PHPExcel_Chart_DataSeries::TYPE_STOCKCHART)) {
                $objWriter->startElement('c:spPr');
                $objWriter->startElement('a:ln');
                if (!empty($plotGroup->customStyle['lineWidth'])) {
                    $objWriter->writeAttribute('w', $plotGroup->customStyle['lineWidth']);
                } else {
